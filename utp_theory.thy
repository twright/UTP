...
definition pred_lattice :: "('\<alpha> pred) gorder" ("\<P>") where
"pred_lattice = \<lparr> carrier = UNIV, eq = (=), le = (\<sqsubseteq>) \<rparr>"
...
(* Is this the same as ref_lattice? *)
interpretation pred_lattice: complete_lattice \<P>
proof (unfold_locales, simp_all add: pred_lattice_def)
  ...
  show "\<exists>s. is_lub \<lparr>carrier = UNIV, eq = (=), le = (\<sqsubseteq>)\<rparr> s A"
    ...
    by (auto intro: Inf_greatest simp add: Inf_lower Upper_def ref_lattice.Sup_upper ref_lattice.Sup_le_iff )
  show "\<exists>i. is_glb \<lparr>carrier = UNIV, eq = (=), le = (\<sqsubseteq>)\<rparr> i A"
    ...
    by (auto intro: Sup_least simp add: Sup_upper Lower_def ref_lattice.Inf_lower ref_lattice.le_Inf_iff)
qed

lemma upred_weak_complete_lattice [simp]: "weak_complete_lattice \<P>"
  by (simp add: pred_lattice.weak.weak_complete_lattice_axioms)

lemma upred_lattice_eq [simp]:
  "(.=\<^bsub>\<P>\<^esub>) = (=)"
  by (simp add: pred_lattice_def)

lemma upred_lattice_le [simp]:
  "le \<P> P Q = (P \<sqsubseteq> Q)"
  by (simp add: pred_lattice_def)

lemma pred_lattice_carrier [simp]:
  "carrier \<P> = UNIV"
  by (simp add: pred_lattice_def)

lemma Healthy_fixed_points [simp]: "fps \<P> H = \<lbrakk>H\<rbrakk>\<^sub>H"
  by (simp add: fps_def pred_lattice_def Healthy_def)
    
lemma upred_lattice_Idempotent [simp]: "Idem\<^bsub>\<P>\<^esub> H = Idempotent H"
  using pred_lattice.weak_partial_order_axioms by (auto simp add: idempotent_def Idempotent_def)

lemma upred_lattice_Monotonic [simp]: "Mono\<^bsub>\<P>\<^esub> H = Monotonic H"
  using pred_lattice.weak_partial_order_axioms by (auto simp add: isotone_def mono_def pred_refine_iff)

subsection \<open> UTP theories hierarchy \<close>

definition utp_order :: "'\<alpha> health \<Rightarrow> '\<alpha> pred gorder" where
...
lemma mono_Monotone_utp_order:
  ...
  using Monotonic_refine by blast

lemma isotone_utp_orderI: "Monotonic H \<Longrightarrow> isotone (utp_order X) (utp_order Y) H"
  by (auto simp add: mono_def isotone_def utp_weak_partial_order pred_ref_iff_le)
...
lemma utp_order_fpl: "utp_order H = fpl \<P> H"
  by (auto simp add: utp_order_def pred_lattice_def fps_def Healthy_def)
...
locale utp_theory =
  fixes hcond :: "'\<alpha> pred \<Rightarrow> '\<alpha> pred" ("\<H>")
  assumes HCond_Idem: "\<H>(\<H>(P)) = \<H>(P)"
begin

  abbreviation thy_order :: "'\<alpha> pred gorder" where
...
lemma upred_lattice_inf:
  ...
  apply (metis Sup_least Sup_upper UNIV_I antisym_conv subsetI pred_lattice.weak.inf_greatest pred_lattice.weak.inf_lower pred_lattice_carrier upred_lattice_le)
*)
...
begin
  ...
  lemma LFP_healthy_comp: "\<^bold>\<mu> F = \<^bold>\<mu> (Fun.comp F \<H>)"
  proof -
...
proof -
  ...
  hence "complete_lattice (utp_order \<H>)"
    by (simp add: utp_order_def, simp add: pred_lattice_def)
...
lemma healthy_inf:
  ...
  shows "\<Sqinter> A = \<H> (\<Sqinter> A)"
  using Knaster_Tarski_idem_inf_eq[OF upred_weak_complete_lattice, of "\<H>"]
  by (simp, metis HCond_Idempotent HCond_Mono assms partial_object.simps(3) pred_lattice_def pred_lattice_inf utp_order_def)
*)
...
[pred]: "utp_star P = (P\<^sup>\<star> ;; utp_unit)"
